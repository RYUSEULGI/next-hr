...
export default async function ContentInfoBox({ id }: Props) {
  ...
  return (
    <article className="flex flex-col">
      <h2 className="text-6xl font-semibold">{content.name}</h2>
      {content.enName && <h4 className="mt-4 mb-2 text-gray-600 font-medium">{content.enName}</h4>}
      ...
      <p className="max-w-md text-sm text-black">
        대한민국 최고의 명문 로스쿨 교수와 학생들이 전대미문의 사건에 얽히게 되면서 펼쳐지는 캠퍼스
        미스터리와 더불어, 피, 땀, 눈물의 살벌한 로스쿨 생존기를 통해 예비 법조인들이 진정한 법과
        정의를 꺠닫는 과정을 담은 드라마
      </p>
    </article>
  );
